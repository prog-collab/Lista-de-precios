-- ============================================================
--  Descuento en efectivo (5% o 10%, a elección del vendedor) para el
--  medio de pago "Efectivo (contado)" en Vender — mismo patrón que el
--  descuento fijo de −5% que ya existe para transferencia, pero con dos
--  porcentajes a elegir en vez de uno solo.
--  Agrega ventas.contado_descuento_pct y extiende gerente_editar_venta_pago
--  para poder editarlo después de guardada la venta.
-- ============================================================

alter table ventas
  add column if not exists contado_descuento_pct numeric(5,2) not null default 0;
...
create or replace function public.gerente_editar_venta_pago(
  ...
  p_contado_descuento_pct numeric default 0
) returns void
language plpgsql
...
begin
  ...
  update ventas set
    ...
    contado_descuento_pct = coalesce(p_contado_descuento_pct,0),
...
grant execute on function public.gerente_editar_venta_pago(uuid, numeric, numeric, numeric, text, numeric, numeric, numeric, numeric, integer, numeric, numeric, numeric) to authenticated;
